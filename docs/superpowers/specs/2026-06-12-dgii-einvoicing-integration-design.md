...
1. `ecf_dgii` (represented as `ecf_sdd` in the database) with display name **"DGII ECF - SSD"**.
...
- **Form Component:**
  ...
  - Automatically pre-select the default provider when `einvoicing` is enabled (`ecf_sdd` for `DO` and `factur_x` for `FR`).
- **Details Section:**
